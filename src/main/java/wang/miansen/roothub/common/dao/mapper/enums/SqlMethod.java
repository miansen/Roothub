...
public enum SqlMethod {
    ...
    INSERT_NOT_NULL("insertNotNull", "插入一条数据，过滤 null 字段和空字符串。", "<script>\nINSERT INTO %s %s VALUES %s\n</script>"),
    ...
    DELETE_BY_ID("deleteById", "根据主键 ID 删除一条数据", "<script>\nDELETE FROM %s WHERE %s = %s\n</script>"),

    DELETE_BATCH_BY_IDS("deleteBatchIds", "根据主键 ID 集合，批量删除多条数据。", "<script>\nDELETE FROM %s WHERE %s IN %s\n</script>"),
    ...
    UPDATE_BY_ID("updateById", "根据主键 ID 更新一条数据", "<script>\nUPDATE %s %s WHERE %s = %s\n</script>"),

    /**
     * 查
     */
    SELECT_BY_ID("selectById", "根据主键 ID 查询一条数据", "<script>\nSELECT %s FROM %s WHERE %s = %s\n</script>"),

    SELECT_BATCH_BY_IDS("selectBatchIds", "根据主键 ID 集合，批量查询多条数据。", "<script>\nSELECT %s FROM %s WHERE %s IN %s\n</script>"),
